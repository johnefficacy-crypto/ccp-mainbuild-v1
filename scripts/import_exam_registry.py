...
def upsert_organization(
    ...
    *,
    ...
    The lookup is an exact SQL filter on the organizations.short_name column (added by
    migration 169), not the heuristic _abbrev_from_name(stored_full_name) reconstruction
    loop that caused 11 duplicate clusters.  Two calls with identical (short_name, state,
    org_type) always resolve to ONE row regardless of in-memory cache state.
    ...
    if not dry_run:
        ...
        # state=None (central orgs) must match IS NULL; state value matches equality.
        ...
        # Insert new — short_name stored for future exact-match deduplication.
        payload = {
            "name": full_name,
...
def upsert_exam(
    ...
    *,
    ...
    if not dry_run:
        existing = (
            sb.table("exams").select("id,metadata").eq("slug", slug).execute().data or []
        )
        if existing:
            exam_id = existing[0]["id"]
            existing_meta = existing[0].get("metadata") or {}
            update_payload: dict = {}
            if conducting_org_id:
                update_payload["conducting_organization_id"] = conducting_org_id
            # Merge metadata: existing keys win; fill gaps from canonical values
            merged_meta = {
            ...
            if update_payload:
                sb.table("exams").update(update_payload).eq("id", exam_id).execute()
