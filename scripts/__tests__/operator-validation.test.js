...
test('terminal gates may use review_by null and drop the defect requirement', () => {
  const { root, registry } = fixture();
  const gate = registry.tracks[0].gates[0];
  gate.status = 'passed';
  gate.review_by = null;
  gate.runbook = null;
  gate.evidence.push({ path: 'docs/operator-validation/evidence/result.md', recorded_at: '2026-07-15', result: 'passed' });
  gate.defects_found = [];
  gate.defects_fixed = [];
  assert.deepEqual(validateRegistry(registry, { repoRoot: root, now: '2026-07-15T09:00:00Z' }), []);
});
...
test('malformed review timestamps are rejected', () => {
  const { root, registry } = fixture();
  registry.tracks[0].gates[0].review_by = '2026-07-15T09:00Z';
  const errors = validateRegistry(registry, { repoRoot: root, now: '2026-07-15T09:00:00Z' });
  assert.ok(errors.some((error) => error.includes('RFC3339 UTC timestamp')));
});

test('duplicate defect ids in a single list are rejected', () => {
  const { root, registry } = fixture();
  const gate = registry.tracks[0].gates[0];
  gate.defects_found = [
    { id: 'study-01', summary: 'First finding.' },
    { id: 'study-01', summary: 'Duplicate id reused for a different finding.' },
  ];
  const errors = validateRegistry(registry, { repoRoot: root, now: '2026-07-15T09:00:00Z' });
  assert.ok(errors.some((error) => error.includes('duplicate defects_found id on gate study-live-gate')));
});

test('malformed defect ids are rejected', () => {
  const { root, registry } = fixture();
  registry.tracks[0].gates[0].defects_found = [{ id: 'Study_01', summary: 'Non kebab-case id.' }];
  registry.tracks[0].gates[0].defects_fixed = [];
  const errors = validateRegistry(registry, { repoRoot: root, now: '2026-07-15T09:00:00Z' });
  assert.ok(errors.some((error) => error.includes('.id must be kebab-case')));
});

test('empty defect arrays pass validation and render as an em dash', () => {
  const { root, registry } = fixture();
  const gate = registry.tracks[0].gates[0];
  gate.defects_found = [];
  gate.defects_fixed = [];
  ...
  const row = rendered.split('\n').find((line) => line.includes('study-live-gate'));
  assert.ok(row);
  assert.match(row, /\|\s+—\s+\|\s+—\s+\|/);
});
...
test('editing a linked implementation contract alone does not force a registry update', () => {
  const { registry } = fixture();
  // docs/status/track.md is an implementation_ref, not a runbook/evidence record.
  assert.deepEqual(validateChangedFiles(['docs/status/track.md'], registry), []);
});
