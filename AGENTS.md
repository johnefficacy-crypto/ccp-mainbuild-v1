...
- Once a gate is migrated, do not mirror its operator status back into global or track-specific checklists.
...
For live operator status, read the generated `docs/operator-validation/INDEX.md` before the large `docs/status/career-copilot-checklist.md`; the checklist is implementation/product-decision history, not a live operator-gate mirror.
