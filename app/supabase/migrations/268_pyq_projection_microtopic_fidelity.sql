...
-- verified primary tag's `topic_id` verbatim. `public.topics` is a two-level
-- tree (`parent_topic_id`, migration 029:31-42), so a primary tag pointing at a
-- MICROTOPIC was flattened: the microtopic id landed in `topic_id` and
-- `mock_question_bank.microtopic_id` (present since migration 135:64) stayed
-- NULL. Every microtopic-tagged question therefore reached the mock bank with
-- no microtopic, and with a `topic_id` that is not a top-level topic — so
-- topic-level rollups double-counted against a child id.
--
-- FIX: resolve the tag's level from `topics.parent_topic_id` and split it:
--   * parent_topic_id IS NULL  -> topic_id = tag topic_id, microtopic_id = NULL
--   * parent_topic_id NOT NULL -> microtopic_id = tag topic_id,
--                                 topic_id = that row's parent_topic_id
-- `microtopic_id` is added to the INSERT column list / VALUES and to the UPDATE
-- branch. The `topics` row is already SELECTed into `v_topic` for the existing
-- active/subject gates, so `parent_topic_id` costs no extra read.
--
-- HASH CHANGE (deliberate, and it re-projects every existing row once):
-- `microtopic_id` is appended to the content hash as a new trailing top-level
-- field, NULL-safe via `coalesce(v_microtopic_id::text, '')` — matching how
-- every other nullable field in this expression is written. The hash must
-- commit to every value written to the bank row; microtopic_id is now one of
-- them, so a re-projection whose only difference is the microtopic re-hashes
-- rather than matching the old row and keeping the stale level.
--
-- Scope note, stated plainly: the hash ALREADY carried every verified tag's
-- topic_id (the tag aggregate), so a tag MOVE between levels was already
-- detected before this migration. Appending microtopic_id makes the hash
-- honest about what is written, but it does NOT close the remaining gap — a
-- topics-tree RE-PARENT (the tag is unchanged, so the tag aggregate is
-- unchanged, yet the resolved topic_id would change). Closing that needs the
-- resolved `v_topic_id` in the hash as well; that is a wider re-projection
-- and is deliberately not taken here.
--
-- Already-projected rows do NOT reproduce their old hash. Appending a field
-- appends a chr(29) separator to the hashed string, so the digest changes even
-- when microtopic_id IS NULL — `coalesce` makes the input well-formed, it
-- cannot make a longer string hash to the same value. The one-time effect: the
-- next sync of each existing row returns 'updated' instead of 'unchanged' and
-- rewrites identical content plus the new fingerprint. This is idempotent —
-- the row is stable from the following sync onward. No backfill is performed
-- or required by this migration.
--
-- LOCKSTEP (three-way, per migration 229): the SQL hash here and
-- compute_content_hash() in app/backend/app/admin/pyq_mock_projection.py are
-- hand-maintained mirrors and are changed together in this same change. The
-- field SET and field ORDER from 239 are preserved verbatim; microtopic_id is
-- APPENDED after the last existing field (the stimulus aggregate), never
-- inserted mid-list.
--
-- 183/184/186/187/229/239/252 are MERGED and IMMUTABLE. This is a forward
-- migration that `create or replace`s ONLY the projection RPC, with the 239
-- body otherwise verbatim: same signature (uuid, uuid, text) -> jsonb, same
-- gates and block reasons, same ON CONFLICT (pyq_question_id) DO UPDATE clause,
-- same return shape, same idempotency contract. The invalidation trigger fn
-- fn_invalidate_pyq_projection() is untouched.
--
-- SLOT NOTE (VERIFY DB): 268 is the next free slot by file listing. Confirm
-- against `select max(version)::int + 1 from schema_migrations` before rollout
-- — do not assume from the filename alone.

-- ── Projection RPC (create or replace, 239 body; microtopic split added) ─────
