-- 179_start_attempt_from_blueprint.sql
-- A-PR3 (clean redo): atomic "persist generated blueprint + start an attempt".
--
-- SUPERSEDES migration 178's definition of start_attempt_from_blueprint via
-- CREATE OR REPLACE (178 is left in place as applied history; this migration is
-- self-contained and re-creates the function whether or not 178 ran). The redo
-- adds one hardening guard over 178: a NEW attempt must freeze at least one
-- response row — a zero-question generated attempt is rejected (and rolled back)
...
-- backend on service_role. Single DO block so the pgx extended-query protocol
-- receives one prepared statement (mirrors migration 162's convention).
...
begin
  execute $ddl$
    ...
    begin
        ...
        -- HARDENING (redo): a brand-new generated attempt must freeze at least one
        -- response. An empty p_response_rows would otherwise create a 0-question
        -- attempt that scores trivially. Raising here rolls back the whole
        -- transaction (blueprint + attempt), so nothing is persisted.
        if jsonb_array_length(coalesce(p_response_rows, '[]'::jsonb)) = 0 then
            raise exception
                'start_attempt_from_blueprint: refusing to start a generated '
                'attempt with zero frozen responses (blueprint %)', v_blueprint_id;
        end if;
