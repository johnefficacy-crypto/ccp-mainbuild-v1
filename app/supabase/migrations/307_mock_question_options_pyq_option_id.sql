-- 307_mock_question_options_pyq_option_id.sql
-- EXPL-OPTID-01 — carry the source option id through the PYQ projection so the
-- explanation join is by IDENTITY rather than by POSITION.
--
-- THE BUG THIS CLOSES
-- `pyq_question_explanations.option_rationales` is a jsonb object keyed by
-- `pyq_options.id` (migration 230). The projection has never retained that id:
-- `272_pyq_projection_bytea_cast_fix.sql:547-562` selects it into `v_opt_row.id`
-- for the correct-option bookkeeping and then inserts the projected row with a
-- FRESH `gen_random_uuid()`, writing only content columns. So the rationale keys
-- resolve to nothing anywhere in the frozen attempt snapshot.
--
-- EXPL-READ-01 (PR #1189) worked around this by re-deriving a POSITIONAL index on
-- both sides, reproducing the projection's own ordering
--
--     row_number() over (order by option_label, id) - 1
--
-- across the question's `reviewer_status='verified'` options. That is correct
-- today and silently wrong the moment the ordering changes, an option is inserted
-- or unverified, or a question is re-projected against a different option set: a
-- rationale then renders against the WRONG option, with nothing to detect it.
-- 394 explanations already depend on that derivation and roughly 2,000 more are
-- planned, so the position is load-bearing for content that asserts why an answer
-- is right.
--
-- WHAT THIS MIGRATION DOES
--   1. Adds `mock_question_options.pyq_option_id`, nullable, FK to pyq_options.
--   2. Indexes it (partial, where not null) — see note B.
--   3. Backfills it ONLY where the mapping is unambiguous — see note C.
--   4. Re-creates `project_pyq_question_to_mock_bank` so every future projected
--      option row carries its source id.
--
-- Note A — NULLABLE, and why it must stay that way.
-- `mock_question_options` rows are also written by paths that have no source
-- option at all: the admin question editor (`app/backend/app/admin/mock_questions.py:263,379`)
-- and the admin CSV/JSON import (`app/backend/app/admin/mock_import.py:485`).
-- Those are authored questions, not projections; null is the correct value there
-- and always will be. Null is also the honest value for any already-projected row
-- whose source cannot be identified with certainty (note C). A NOT NULL
-- constraint is therefore not a future tightening to aim for on this column.
--
-- Note B — `on delete set null`, NOT cascade, and why the index exists.
-- `mock_question_stimuli.pyq_stimulus_id` (229:47-49) is deliberately lineage
-- only, with no FK, so that deleting a source row can never delete a projected
-- snapshot row. This column keeps that intent while still getting referential
-- integrity: `on delete set null` means deleting a `pyq_options` row degrades the
-- join back to the positional fallback, which is exactly today's behaviour, and
-- never destroys a bank row an in-flight attempt depends on. `on delete cascade`
-- would have deleted projected options out from under live attempts.
-- The partial index is not for the read path — the review path reads
-- `pyq_option_id` out of the FROZEN question_snapshot, never by querying this
-- column — it is so the FK's own `set null` enforcement does not sequential-scan
-- `mock_question_options` on every `pyq_options` delete.
--
-- Note C — the backfill refuses to guess.
-- It writes a row ONLY when all three hold:
--   (i)   the projected question has PYQ lineage (`mock_question_bank.pyq_question_id`),
--   (ii)  the count of verified `pyq_options` for that source question equals the
--         count of `mock_question_options` on the projected question — a count
--         mismatch means the option set moved since projection, so no position is
--         trustworthy, and
--   (iii) at the matched position, `option_text` is identical on both sides.
-- The positional candidate is unique by construction (`row_number()` over a
-- partition yields exactly one row per index), so (iii) is the real test: it is
-- the independent evidence that the position still means what it meant at
-- projection time. Anything failing any of the three is left NULL and continues
-- to use the positional fallback, which is no worse than before this migration.
-- Nothing is overwritten: the update touches only rows already NULL.
--
-- DRY RUN — run this BEFORE applying, to see what the backfill would do.
-- It is the same logic as the UPDATE below, counting instead of writing.
--
--   with src as (
...
--   )
--   select count(*) filter (where s.pyq_option_id is not null) as would_populate,
...
--
-- Plus the rows with no PYQ lineage at all, which this migration never touches:
--   select count(*) from public.mock_question_options mo
--   join public.mock_question_bank q on q.id = mo.question_id
--   where q.pyq_question_id is null;
--
-- NO HASH CHANGE. `pyq_option_id` is lineage, not content, and is not part of the
-- projection's content-hash term (`272:363`) or its Python mirror
-- (`app/backend/app/admin/pyq_mock_projection.py:243-276`). Already-projected rows
-- keep their hash and stay "unchanged", so this migration does NOT trigger a
-- re-sync of every paper. Rows only pick up `pyq_option_id` from the backfill, or
-- from the next projection they were going to get anyway.
--
-- SLOT NOTE: 307 = MAX(filesystem) + 1 after 306_gir_english_ssc_item_type_gaps.sql.
...
alter table public.mock_question_options
  add column if not exists pyq_option_id uuid
    references public.pyq_options(id) on delete set null;

comment on column public.mock_question_options.pyq_option_id is
  'Lineage: the pyq_options row this projected option was built from. NULL for '
  'authored/imported options, and for projected rows whose source could not be '
  'identified unambiguously. Keyed on by pyq_question_explanations.option_rationales.';

-- ── 2. Index ──────────────────────────────────────────────────────────────────
-- Partial: the column is NULL on every authored row, and those are never the
-- target of an FK enforcement scan.

create index if not exists idx_mock_question_options_pyq_option
  on public.mock_question_options(pyq_option_id)
  where pyq_option_id is not null;

-- ── 3. Backfill (unambiguous rows only — note C) ──────────────────────────────

with src as (
  select o.id               as pyq_option_id,
         o.question_id      as pyq_question_id,
  ...
  select mo.id           as mock_option_id,
         mo.option_index  as option_index,
  ...
  join public.mock_question_bank q on q.id = mo.question_id
  where q.pyq_question_id is not null
    and mo.pyq_option_id is null
)
update public.mock_question_options mo
set    pyq_option_id = s.pyq_option_id
from   tgt t
join   src s
  on  s.pyq_question_id = t.pyq_question_id
  and s.opt_idx         = t.option_index
  -- count guard: the option set has not changed size since projection
  and s.src_n           = t.tgt_n
  -- text guard: the independent evidence that the position still means what it
  -- meant at projection time. is not distinct from, so NULL matches NULL.
  and s.option_text is not distinct from t.option_text
where  mo.id = t.mock_option_id
  and  mo.pyq_option_id is null;

-- ── 4. Projection RPC ─────────────────────────────────────────────────────────
-- 272 is MERGED and IMMUTABLE. This is a forward migration that
-- `create or replace`s the projection RPC with 272's body verbatim; the ONLY
-- change is the option insert at the loop below, which now also writes
-- pyq_option_id. Field set, field order, separators and the content-hash term are
-- otherwise untouched.
...
begin
    ...
    loop
        -- EXPL-OPTID-01: carry the SOURCE option id onto the projected row.
        -- Every other value in this insert is content; pyq_option_id is lineage,
        -- and it is the only one that survives a re-ordering of the options.
