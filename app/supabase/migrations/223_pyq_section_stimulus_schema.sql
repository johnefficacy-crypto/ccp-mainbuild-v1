...
--
-- Checkpost review (PR #892), first pass, fixes:
...
--
-- Checkpost review, second pass, fixes:
--   P0-1b linked section reassignment: changing section_id on an already-linked
--        question or stimulus did not re-check the existing pyq_question_stimuli
--        link (the link row itself doesn't change, so its own trigger never
--        fires). The question/stimulus section triggers now also re-validate
--        any existing links against the new section_id.
--   P0-2b NULL-unsafe move checks: the parent-move revalidation functions used
--        `<>` to compare exam_phase_id/pyq_paper_id, which is NULL for every row
--        when the new value is NULL, silently skipping the EXISTS check. All
--        such comparisons now use `IS DISTINCT FROM`.
--   P0-3  concurrent-write races: the validation/revalidation functions read
--        parent rows with a plain SELECT, which is a check-then-act race under
--        concurrent writes touching the same rows. Every read that a write path
--        depends on now takes an explicit row lock (FOR SHARE / FOR SHARE OF) on
--        the specific parent/child rows involved, so a concurrent statement
--        touching the same row blocks until this transaction commits and
--        re-validates against final state. Note the question-section and
--        stimulus-section validators lock in opposite orders (question-then-
--        linked-stimuli vs. stimulus-then-linked-questions): a concurrent
--        question-section edit and stimulus-section edit on the same link can
--        deadlock. That is a safe outcome — Postgres detects it and aborts one
--        transaction (the caller retries) rather than allowing inconsistent
--        committed state — but it means this is a mitigation, not a proof of
--        full serializability across arbitrary concurrent write mixes. A
--        two-session concurrency regression against a live instance is tracked
--        as a VERIFY DB follow-up in the checklist rather than asserted here.
--   P1b   stimulus association trust: pyq_question_stimuli now carries its own
--        reviewer_status/reviewed_by/reviewed_at (mirroring
--        pyq_question_topic_tags), defaulting to 'pending' on every new link and
--        forced back to 'pending' on repoint (question_id/stimulus_id change),
--        so the question<->stimulus association is independently governed
--        rather than inheriting trust from either endpoint. Editing a verified
--        stimulus's content also forces it back to 'needs_correction'. The
--        atomic cross-cascade RPC (question review approving its stimulus
--        links, mirroring update_pyq_question_review_atomic in migration 162)
--        is backend-endpoint work and stays scoped to PR-3 (admin review).
...
-- Editing a verified stimulus's learner-visible content must not silently
-- leave it verified (checkpost P1b) — mutating content_text, stimulus_type,
-- language or metadata on an already-verified row forces it back to
-- 'needs_correction', unless the same statement also explicitly changes
-- reviewer_status (e.g. an operator's review-plus-correction in one write).
...
create trigger trg_pyq_stimuli_downgrade_on_content_edit
  before update of content_text, stimulus_type, language, metadata on public.pyq_stimuli
...
-- reviewer_status/reviewed_by/reviewed_at review-gate the ASSOCIATION itself
-- (mirroring pyq_question_topic_tags, migration 032) rather than inheriting
-- trust from either endpoint: a link is 'pending' until an operator confirms
-- the passage/chart shown for this question is actually the right one, and a
-- repoint (question_id or stimulus_id change) forces it back to 'pending'.
...
create index if not exists idx_pyq_question_stimuli_review
  on public.pyq_question_stimuli(reviewer_status);
...
-- 1. pyq_questions.section_id must share its paper's exam_phase_id, AND any
--    existing pyq_question_stimuli link for this question must remain
--    section-compatible with the new section_id (closes the "reassign a
--    linked question's section" hole: the link row itself doesn't change, so
--    its own trigger never fires on this write path).
...
declare
  ...
  v_section_changed boolean;
begin
  if new.section_id is not null then
    select exam_phase_id into v_paper_phase
      from public.pyq_papers where id = new.pyq_paper_id for share;
  ...
  -- OLD is only assigned on UPDATE; on INSERT there cannot be a pre-existing
  -- pyq_question_stimuli row for this not-yet-created question, so always
  -- run the (harmlessly no-op) check rather than dereference OLD unassigned.
  if tg_op = 'UPDATE' then
    v_section_changed := (new.section_id is distinct from old.section_id);
  else
    v_section_changed := true;
  end if;

  if not v_section_changed then
    return new;
  end if;
  ...
  if new.section_id is not null and exists (
    ...
    where qs.question_id = new.id
      and st.section_id is not null
      and st.section_id <> new.section_id
    for share of st
  ) then
    raise exception
      'pyq_questions % section_id change to % would break an existing pyq_question_stimuli section compatibility',
      new.id, new.section_id;
  end if;
...
-- 2. pyq_stimuli.section_id must share its paper's exam_phase_id, AND any
--    existing pyq_question_stimuli link for this stimulus must remain
--    section-compatible with the new section_id (mirrors fix #1 above).
...
begin
  ...
  if new.section_id is not null and exists (
    ...
    where qs.stimulus_id = new.id
      and q.section_id is not null
      and q.section_id <> new.section_id
    for share of q
  ) then
    raise exception
      'pyq_stimuli % section_id change to % would break an existing pyq_question_stimuli section compatibility',
      new.id, new.section_id;
  end if;
...
--    sides carry a section, the same section. A repoint (question_id or
--    stimulus_id change on an existing link) forces the link's own review
--    state back to 'pending' — repointing an already-reviewed association is
--    exactly the kind of change that must be re-reviewed (checkpost P1b).
...
begin
  select pyq_paper_id, section_id into v_question_paper, v_question_section
    from public.pyq_questions where id = new.question_id for share;
  select pyq_paper_id, section_id into v_stimulus_paper, v_stimulus_section
    from public.pyq_stimuli where id = new.stimulus_id for share;
  ...
  if tg_op = 'UPDATE' then
    v_repointed := (new.question_id is distinct from old.question_id)
                or (new.stimulus_id is distinct from old.stimulus_id);
    if v_repointed then
      new.reviewer_status := 'pending';
      new.reviewed_by := null;
      new.reviewed_at := null;
    end if;
  end if;
...
begin
  ...
  if exists (
    ...
    where q.section_id = new.id and p.exam_phase_id is distinct from new.exam_phase_id
    for share of q
  ) then
    raise exception
  ...
  if exists (
    ...
    where s.section_id = new.id and p.exam_phase_id is distinct from new.exam_phase_id
    for share of s
  ) then
    raise exception
...
begin
  ...
  if exists (
    ...
    where q.pyq_paper_id = new.id and s.exam_phase_id is distinct from new.exam_phase_id
    for share of q
  ) then
    raise exception
  ...
  if exists (
    ...
    where st.pyq_paper_id = new.id and s.exam_phase_id is distinct from new.exam_phase_id
    for share of st
  ) then
    raise exception
...
begin
  ...
  if exists (
    ...
    from public.pyq_question_stimuli qs
    join public.pyq_stimuli st on st.id = qs.stimulus_id
    where qs.question_id = new.id and st.pyq_paper_id is distinct from new.pyq_paper_id
    for share of st
  ) then
    raise exception
...
begin
  ...
  if exists (
    ...
    from public.pyq_question_stimuli qs
    join public.pyq_questions q on q.id = qs.question_id
    where qs.stimulus_id = new.id and q.pyq_paper_id is distinct from new.pyq_paper_id
    for share of q
  ) then
    raise exception
