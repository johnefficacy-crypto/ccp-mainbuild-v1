...
status from live DB state.  It is designed to be fail-soft: any individual
step that encounters a read error sets status="missing" and continues.
...
def compute_cycle_readiness(
    ...
    try:
        ...
        logger.warning("cycle_readiness step1 error: %s", exc)
        s1 = _step(1, "cycle_details", "Cycle details", _MISSING,
                   gate_class="hard", evidence_scope="selected_cycle")
    steps.append(s1)
    ...
    try:
        if not cycle_id:
            s2 = _step(2, "phases_schedule", "Phases schedule", _NA,
                       gate_class="hard", evidence_scope="selected_cycle",
                       not_applicable_reason="no_cycle_selected")
        else:
        ...
        logger.warning("cycle_readiness step2 error: %s", exc)
        s2 = _step(2, "phases_schedule", "Phases schedule", _MISSING,
                   gate_class="hard", evidence_scope="selected_cycle")
    steps.append(s2)
    ...
    try:
        ...
        logger.warning("cycle_readiness step3 error: %s", exc)
        s3 = _step(3, "source_documents", "Source documents", _MISSING,
                   gate_class="advisory", evidence_scope="exam_wide")
    steps.append(s3)
    ...
    try:
        ...
        logger.warning("cycle_readiness step4 error: %s", exc)
        s4 = _step(4, "extraction", "Text extraction", _MISSING,
                   gate_class="advisory", evidence_scope="exam_wide")
    steps.append(s4)
    ...
    try:
        ...
        logger.warning("cycle_readiness step5 error: %s", exc)
        s5 = _step(5, "syllabus_mapping", "Syllabus mapping", _MISSING,
                   gate_class="hard", evidence_scope="mixed")
    steps.append(s5)
    ...
    try:
        ...
        logger.warning("cycle_readiness step6 error: %s", exc)
        s6 = _step(6, "pyq_readiness", "PYQ readiness", _MISSING,
                   gate_class="advisory", evidence_scope="exam_wide")
    steps.append(s6)
    ...
    try:
        ...
        logger.warning("cycle_readiness step8 error: %s", exc)
        s8 = _step(8, "competition_context", "Competition context", _MISSING,
                   gate_class="advisory", evidence_scope="exam_wide")
    steps.append(s8)
    ...
    try:
        ...
        logger.warning("cycle_readiness step9 error: %s", exc)
        s9 = _step(9, "review_activate", "Review & activate", _MISSING,
                   gate_class="hard", evidence_scope="exam_wide")
    steps.append(s9)
