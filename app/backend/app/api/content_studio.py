...
is carried by `writing_prompt_targets`. This router is the operator write path:
  ...
  Exam Assignments        → writing_prompt_targets set/list/remove
                             (exam_intelligence.manage — Manage Exam owns
                              applicability, §1.1)
...
PERM_ASSIGN = EXAM_INTELLIGENCE_MANAGE  # applicability is Manage Exam's remit
...
def _require_content_read(user: dict = Depends(get_current_user)) -> dict:
    ...
    if perms & {PERM_AUTHOR, PERM_REVIEW, PERM_ASSIGN}:
        return user
    raise HTTPException(status_code=403,
                        detail="Missing permission: content_studio.author/review or exam_intelligence.manage")
...
class WritingPromptTargetBody(BaseModel):
    model_config = ConfigDict(extra="forbid")
    reason: str = Field(..., min_length=8, max_length=500)
    ...
    applicability_status: Literal["active", "excluded", "pending_review"] = "active"
...
def set_writing_prompt_target(
    prompt_id: str,
    body: WritingPromptTargetBody,
    admin: dict = Depends(require_permission(PERM_ASSIGN)),
    __: None = Depends(_flag_enabled),
) -> dict[str, Any]:
    supabase = get_supabase_admin()
    try:
        result = supabase.rpc("cms_set_writing_prompt_target", {
            "p_prompt_id": prompt_id,
            ...
            "p_status": body.applicability_status,
        ...
        raise _map_rpc_error(exc, "set_writing_prompt_target") from exc
    return {"ok": True, "result": _rpc_row(result)}
