"""Schema-contract tests for migration 241
...
241 grants SELECT only, preserving the 183 posture that writes flow solely
...
MIGRATION = (
    _MIGRATIONS / "241_pyq_projection_service_role_read_grant.sql"
).read_text()
...
def test_does_not_grant_write_privileges():
    ...
    for verb in ("insert", "update", "delete", "grant all"):
        assert f"grant {verb}" not in low, f"241 must not grant {verb} on the projection table"
