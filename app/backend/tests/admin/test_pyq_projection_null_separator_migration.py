"""Regression schema-contract tests for migration 239
...
the instant it reached the hash expression. Migration 239 `create or replace`s
...
(Authored as 238; renumbered 238 -> 239 after slot 238 was taken by
238_ewp_rollup_completed_at.sql on apply.)
...
fix lives only in the forward migration (239).
"""
from pathlib import Path

_MIGRATIONS = Path(__file__).resolve().parents[3] / "supabase" / "migrations"

MIGRATION = (_MIGRATIONS / "239_pyq_projection_null_separator_fix.sql").read_text()
...
def test_hash_expression_uses_group_separator_top_level():
    ...
    # 239 swaps every one of them to chr(29). A drift here means a field was
...
def test_within_list_separators_unchanged():
    ...
    expr_239 = _hash_expression(MIGRATION)
    expr_229 = _hash_expression(MIG_229)
    assert expr_239.count("chr(30)") == expr_229.count("chr(30)")
    assert expr_239.count("chr(31)") == expr_229.count("chr(31)")


def test_field_set_and_order_preserved_from_229():
    # Pure separator swap: replacing chr(0) with chr(29) in the 229 hash
    # expression must reproduce the 239 hash expression verbatim. This proves no
...
def test_invalidation_trigger_fn_not_rewritten():
    # fn_invalidate_pyq_projection() does not use chr(0); 239 must not re-create
