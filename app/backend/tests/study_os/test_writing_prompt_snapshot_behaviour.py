...
and proves what 222 changes:
  ...
  1. Immutable prompt snapshot — ``ewp_create_writing_session`` freezes the
     ...
     min_words, max_words, rubric_dimensions) onto
     ``writing_sessions.prompt_snapshot``; a later edit of the underlying prompt
     does NOT change what the claim payload sees, and a direct UPDATE of
     prompt_snapshot itself is REJECTED by the extended
     ``ewp_guard_session_snapshot()`` trigger.
  2. exam_id is PINNED at session creation — resolved once from the launch
     study_task (the same context validated for prompt applicability) onto
     ``writing_sessions.exam_id``/``exam_phase_id``, NOT re-derived from
     ``study_tasks`` at claim/drain time. A study_task's exam_id changing (or
     the task being deleted) AFTER session creation must NOT change the exam
     attributed to evaluation/mastery evidence. The pinned columns are also
     guarded immutable. NULL for an ad-hoc session with no task.
...
def _swap_dbname(dsn: str, name: str) -> str:
    """Same server/creds as dsn, targeting database `name`. CI's `EWP_PG_DSN`
    points at one shared `ewp_it` used by every test module in the backend
    job; this module applies migration 214 (drops writing_prompts.exam_id
    permanently) which would otherwise contaminate sibling modules that assume
    the pre-214 shape. Run this module's whole chain in its own disposable DB."""
    if dsn.startswith("postgresql://") or dsn.startswith("postgres://"):
        scheme, rest = dsn.split("://", 1)
        head, _, tail = rest.partition("/")
        qs = tail.split("?", 1)
        suffix = f"?{qs[1]}" if len(qs) > 1 else ""
        return f"{scheme}://{head}/{name}{suffix}"
...
CREATE TABLE IF NOT EXISTS public.study_tasks (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid NOT NULL, task_type text, exam_id uuid REFERENCES public.exams(id), exam_phase_id uuid REFERENCES public.exam_phases(id));
...
INSERT INTO exams(id) VALUES ('{_EXAM2}') ON CONFLICT DO NOTHING;
...
INSERT INTO writing_prompts(id,subject_id,topic_id,microtopic_id,exercise_type,prompt_text,source_text,required_words,min_words,max_words,difficulty_level,reviewer_status,is_active,required_sentence_count)
  SELECT '{_PROMPT}',
    ...
    'Correct the grammatical error in the sentence.','He go to school every day.',
    '["school"]'::jsonb,3,20,1,'verified',false,1
...
def _apply():
    global _DSN
    maint = _swap_dbname(_DSN, "postgres")
    subprocess.run(
        [_PSQL, maint, "-v", "ON_ERROR_STOP=1", "-X", "-q",
         "-c", f"DROP DATABASE IF EXISTS {_PRIVATE_DB};",
         "-c", f"CREATE DATABASE {_PRIVATE_DB};"],
        check=True, capture_output=True, text=True,
    )
    _DSN = _swap_dbname(_DSN, _PRIVATE_DB)
    _psql(_BOOTSTRAP)
...
def test_create_session_captures_immutable_prompt_snapshot():
    ...
    assert snap["required_words"] == ["school"]
    assert snap["min_words"] == 3 and snap["max_words"] == 20


def test_create_session_pins_exam_from_study_task():
    sid = _create_session(_TASK)
    row = _scalar(f"SELECT exam_id FROM writing_sessions WHERE id='{sid}'")
    assert row == _EXAM


def test_create_session_has_null_exam_with_no_study_task():
    sid = _create_session(None)
    row = _scalar(f"SELECT COALESCE(exam_id::text,'') FROM writing_sessions WHERE id='{sid}'")
    assert row == ""


def test_guard_rejects_prompt_snapshot_mutation():
    sid = _create_session(_TASK)
    p = _psql(
        f"UPDATE writing_sessions SET prompt_snapshot = prompt_snapshot || "
        f"'{{\"prompt_text\":\"HACKED\"}}'::jsonb WHERE id='{sid}'",
        expect_ok=False)
    assert "session_snapshot_immutable" in p.stderr


def test_guard_rejects_pinned_exam_mutation():
    sid = _create_session(_TASK)
    p = _psql(
        f"UPDATE writing_sessions SET exam_id='{_EXAM2}' WHERE id='{sid}'",
        expect_ok=False)
    assert "session_snapshot_immutable" in p.stderr


def test_exam_stays_pinned_after_study_task_exam_changes():
    sid = _create_session(_TASK)
    # Mutate the STUDY TASK's exam_id after the session was created.
    _psql(f"UPDATE study_tasks SET exam_id='{_EXAM2}' WHERE id='{_TASK}'")
    try:
        row = _scalar(f"SELECT exam_id FROM writing_sessions WHERE id='{sid}'")
        assert row == _EXAM  # pinned at creation, not re-derived from the task
        _submit(sid, "pinned exam answer text", "f1" + "0" * 62)
        claim = _json("SELECT ewp_claim_evaluation_job(900, ARRAY['language_evaluation'])")
        assert claim["exam_id"] == _EXAM
    finally:
        _psql(f"UPDATE study_tasks SET exam_id='{_EXAM}' WHERE id='{_TASK}'")


def test_claim_surfaces_pinned_exam_and_prompt_and_source():
    sid = _create_session(_TASK)
...
def test_prompt_edit_after_session_does_not_change_claim_scope():
    ...
    _psql(
        ...
        f"required_words='[\"EDITED\"]'::jsonb, min_words=999, max_words=999 "
        f"WHERE id='{_PROMPT}'")
    try:
        claim = _json("SELECT ewp_claim_evaluation_job(900, ARRAY['language_evaluation'])")
        ...
        _psql(
            ...
            f"source_text='He go to school every day.', required_words='[\"school\"]'::jsonb,"
            f"min_words=3,max_words=20 WHERE id='{_PROMPT}'")
...
def test_full_pipeline_writes_evidence_bound_to_derived_exam():
    ...
    # The mastery drain reads the SAME pinned session column.
