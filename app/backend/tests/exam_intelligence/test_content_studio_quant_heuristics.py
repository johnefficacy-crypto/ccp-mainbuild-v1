"""Router-layer tests for Content Studio quant-heuristic operations (GQR-Q7).

Covers the `/api/admin/content-studio/quant-heuristics` surface added over
migration 243 (`app/api/content_studio.py`) at the FastAPI boundary: the shared
read gate, list filters + pagination, the heuristic-specific reviewer-transition
guard (which DIFFERS from writing prompts), the reopen-verified note rule, and
RPC error-code → HTTP mapping. The atomic RPC *behaviour* (audit rows, CAS,
transition matrix) is proven against real Postgres in the migration-243 backend
tests; here we only prove the router contract.
"""
...
def test_list_readable_by_author_review_manage_and_super_admin():
    for perms, role in (([AUTHOR], "admin"), ([REVIEW], "admin"),
        ...
        sb = CSSBStub(_seed())
        r = _client(sb, permissions=perms, role=role).get(f"{_BASE}/quant-heuristics")
        assert r.status_code == 200, (perms, role, r.text)


def test_list_denied_without_any_content_permission():
    sb = CSSBStub(_seed())
    r = _client(sb, permissions=["some.other"]).get(f"{_BASE}/quant-heuristics")
    assert r.status_code == 403


def test_list_denied_for_anonymous():
    sb = CSSBStub(_seed())
    r = _client(sb, permissions=[REVIEW], role="user", anonymous=True).get(f"{_BASE}/quant-heuristics")
    assert r.status_code == 403
...
def test_list_filters_by_type_and_status():
    ...
    assert {h["id"] for h in r.json()["items"]} == {_HEUR}


def test_list_enriches_topic_name_and_search_matches_name():
    sb = CSSBStub(_seed())
    r = _client(sb).get(f"{_BASE}/quant-heuristics?q=percentage")
    body = r.json()
    assert {h["id"] for h in body["items"]} == {_HEUR}
    assert body["items"][0]["topic_name"] == "Percentages"
    assert body["total"] == 1
    r2 = _client(sb).get(f"{_BASE}/quant-heuristics?q=nomatch")
    assert r2.json()["items"] == []


def test_get_returns_404_when_absent():
    sb = CSSBStub(_seed())
    r = _client(sb).get(f"{_BASE}/quant-heuristics/{_ABSENT}")
    assert r.status_code == 404


def test_get_returns_enriched_row():
    sb = CSSBStub(_seed())
    r = _client(sb).get(f"{_BASE}/quant-heuristics/{_HEUR}")
    assert r.status_code == 200, r.text
    assert r.json()["topic_name"] == "Percentages"
...
def test_review_blocks_illegal_transition_needscorrection_to_verified():
    # Heuristic matrix (differs from writing prompts): needs_correction may go to
    # pending|rejected only — NOT straight to verified.
