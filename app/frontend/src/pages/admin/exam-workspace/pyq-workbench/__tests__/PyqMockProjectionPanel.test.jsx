...
test("renders complete, truthful projection remediation", async () => {
  ...
  const data = report([
    ...
    row("stim", "stimulus_not_verified", "Unverified passage"),
  ]);
  ...
  expect(summary.textContent).toMatch(/1\s*Multiple verified primary tags/);
  expect(summary.textContent).toMatch(/1\s*No verified correct option/);
  ...
  expect(screen.getByTestId("preview-row-stim").textContent).toMatch(/Linked passage\/stimulus is not verified/);
...
test("drops stale status, preview and sync after a paper change", async () => {
  ...
  api.get.mockImplementation((url) => {
    if (url.includes("/pyq-papers/a/") && url.endsWith("/status")) return aStatus.promise;
    if (url.includes("/pyq-papers/a/") && url.endsWith("/preview")) return aPreview.promise;
