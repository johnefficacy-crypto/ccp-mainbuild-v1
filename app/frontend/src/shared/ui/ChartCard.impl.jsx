...
export default function ChartCardImpl({ title, subtitle, data = [], emptyMessage = "No data available." }) {
  return (
    <div className="lg:col-span-2 soft-card rounded-2xl p-5">
      ...
      {data.length === 0 ? (
        <div className="mt-4 text-sm text-muted-foreground">{emptyMessage}</div>
      ) : (
        <div className="h-48 mt-5">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data}>
              ...
              <Tooltip />
              <Area type="monotone" dataKey="h" stroke="hsl(var(--primary))" strokeWidth={2} fill="hsl(var(--accent))" fillOpacity={0.45} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}
