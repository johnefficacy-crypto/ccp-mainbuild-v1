...
| BUG-EI-2 `GET /console/exams/{id}` → 500 | **OPEN BUG / P0 / DISPATCH NOW** | `console_detail.py::_documents()` queries `document_assets` with `.eq("exam_id", ...)` and `.select("id, extraction_status")` — neither column exists on that table. Same bug at `readiness.py:77`. **Design decision LOCKED (2026-06-21):** canonical extraction signal is `document_processing_jobs` where `job_type='text_extract'` and latest `status` determines succeeded/pending/failed/needs_review/not_started. `syllabus_documents.trust_status='verified'` is a human-review gate, orthogonal to extraction — NOT a valid proxy. Option A (trust_status proxy) undercounts. Implementation requires: `document_assets` ownership from metadata; exam/cycle filtering from metadata; batched `document_processing_jobs` loading; latest job per asset; shared logic between `console_detail.py` and `readiness.py`. Fix class: backend-only. See `docs/audits/document-readiness-2026-06-21.md`. |
...
| ID | Area | Status | Notes |
|---|---|---|---|
| E1 | KnowledgeGovernance adds 5th path to exam setup, zero metrics | **LOCKED — REMOVE; I7 READY TO DISPATCH** | Decision locked 2026-06-21: remove exam lane/card from `KnowledgeGovernance.jsx`; update landing-page count/copy from 4 to 3 lanes; update landing-page tests. Do NOT rename KG. Do NOT remove sidebar exam group (deferred to I8-A). Do NOT change backend metrics. Console already owns triage; KG lane adds no unique capability. |
