...
| IA design-lock document | **CODE PRESENT IN THIS PR / effective after merge** | `docs/status/Exam-Management-IA-Design-Lock-2026-06-21.md` on branch `docs/exam-management-ia-design-lock` (PR #752). I8 design gate: satisfied once this PR merges. Next implementation prerequisite: backend management read model. I8-A/B/C: serial, single owner. |
...
| Portfolio/readiness read-model (backend) | **GATED** | prerequisite before I8-A can display real data; parallel-safe backend after IA lock PR merges |
...
| Item | Status | Notes |
|---|---|---|
| IA design-lock document | CODE PRESENT IN THIS PR / pending merge — PR #752 | `docs/status/Exam-Management-IA-Design-Lock-2026-06-21.md` on branch `docs/exam-management-ia-design-lock`. All 13 sections locked. Appendix B COMPLETE — no items block I8-A/B/C (all 5 deferred decisions resolved). I8 design gate: satisfied once this PR merges. |
...
| Backend management read-model | PLANNED — GATED (after IA lock PR merges) | New headless endpoints at `/management/exams` (not a UI surface) required by I8 consolidated pages. Contract locked in design-lock doc Section 8: `/management/exams` (paginated list) and `/management/exams/{exam_id}` (detail). Deterministic current-cycle selection rule locked (Section 8.3). Parallel-safe backend work after IA lock PR merges; frontend integration after I8-A. |
