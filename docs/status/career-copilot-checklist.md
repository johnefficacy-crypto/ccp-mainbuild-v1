...
Last repo verification: 2026-06-20 at `11d188ef` (current main / dispatch branch `work`).
...
| CL-5 one-primary-per-screen buttons | CLEANUP PENDING | Exam Registry now has one primary header action: Open console. Create exam is secondary and Advanced import / repair remains tertiary. Remaining locked scopes: B3d-2 Console Work Queue action hierarchy, B3d-3 Guided Exam Wizard primary-action hierarchy, and B3d-close cross-surface closure audit. Rule: a screen may expose at most one screen-level primary CTA; pressed filter states must not use primary-action styling; repeated table-row actions are contextual, not screen primaries; local form submission buttons are scoped to their form/card and are not automatically competing screen-level CTAs. `SetupPanel` redesign remains owned by Lane C and must not be absorbed into B3d. |
...
| CL-6b retire dormant console presentation plumbing | CLEANUP PENDING | Locked scope B4 / CL-6b: `ExamWorkspaceContext`, `OverviewPanel`, `ReviewActivatePanel`, and isolated `ExamPublishImpact` usage/tests still carry the old console-variant contract. `ExamPublishImpact` has no current runtime consumer. Delete, reuse, or redesign in a separate PR without changing active console routes. |
