...
Last repo verification: 2026-06-19 at `refactor/mastery-preview-exact-replay`.
...
| Correction idempotency guard (23505) / atomic persistence | CODE-FIXED, MIGRATION VALIDATION PENDING | PR-5B (fix/correction-draft-atomicity): four data-loss defects fixed atomically. D1: batch-insert 23505 skip replaced by per-draft `ensure_mock_correction_draft` RPC (ON CONFLICT DO NOTHING; non-conflicting rows never lost). D2: delete-before-insert replaced by `replace_manual_mock_correction_drafts` RPC (single transaction; prior drafts preserved on failure). D3: 23505 catch-and-fetch removed; RPC handles conflict internally. D4: `_safe`-wrapped review_state update replaced by atomic RPC step (failure propagates). D5: string-search `"23505" in str(exc)` eliminated; no application-level 23505 handling in either path. Migration 182 adds both RPCs (SECURITY DEFINER, service_role only). Applied/dismissed rows preserved. Empty-draft contract documented and tested: deletes all drafted rows, sets review_state='reviewed'. VERIFY DB: dry-run migration 182 with BEGIN/ROLLBACK before production apply. |
...
| CL-3 remove CMS `+ New guided exam` CTA | CODE PRESENT IN THIS CHECKOUT | Advanced Import / Repair no longer renders the redundant guided-exam CTA. Entity selection, Reload, New row, Bulk import, create-row, Add-cycle and repair workflows remain unchanged. The guided-exam route remains available outside the CMS. |
...
| CL-6b retire dormant console presentation plumbing | CLEANUP PENDING | `ExamWorkspaceContext`, `OverviewPanel`, `ReviewActivatePanel`, and the isolated `ExamPublishImpact` tests still carry the old console-variant contract. `ExamPublishImpact` has no current runtime consumer. Delete, reuse, or redesign in a separate PR. |
