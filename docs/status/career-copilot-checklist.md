...
| Live canary user allowlist | BLOCKED | Hard prerequisite — `FF_MOCK_MASTERY_LIVE_USER_IDS` (or equivalent per-user allowlist) is **not implemented** in deployed code. `MasteryWriter.process_attempt_sync` applies live writes to all users globally; no per-user gate exists. The bounded canary plan (`docs/ops/pr8_live_canary_plan.md`) has been updated to: one named disposable canary UUID (allowlisted) + one named disposable control UUID (not allowlisted, same template), one attempt each, 15-minute window, with exact-UUID-scoped rollback SQL and dry-run checklist. The plan cannot be executed until the allowlist implementation PR merges, deploys, and is validated against the behavior matrix in the plan doc. Do not flip the flag without it. |
