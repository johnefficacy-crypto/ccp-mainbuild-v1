...
| BUG-EI-2 `GET /console/exams/{id}` → 500 | CODE-FIXED, VALIDATION PENDING | Final fix: `load_doc_extraction_counts()` helper (readiness.py) queries `document_assets` (scope='admin_exam_intelligence', exam_id from metadata JSONB) then batch-loads latest `document_processing_jobs` (job_type='text_extract') per asset. Both `console_detail.py::_documents()` (replaced) and `readiness.py::_documents()` now share this helper. `_AREA_ENTITY_KIND["documents"]` updated to `"document_assets"`. 54 tests passing: succeeded-job→extracted; verified-syllabus-doc-alone→NOT extracted; no-job→not_started. See `docs/audits/document-readiness-2026-06-21.md`. |
...
| Document readiness extraction status | CODE-FIXED, VALIDATION PENDING | `load_doc_extraction_counts()` helper in `readiness.py` is the shared source: queries `document_assets` (scope='admin_exam_intelligence', exam_id from metadata) then batch-loads latest text_extract job from `document_processing_jobs`. `console_detail.py` and `readiness.py::_documents()` both call it. `trust_status` on `syllabus_documents` is no longer used as extraction proxy. Regression tests confirm succeeded-job→extracted; verified-syllabus-doc-alone→not extracted. See `docs/audits/document-readiness-2026-06-21.md`. |
