...
|---|---|---|---|
| D1 | Exam identity in 3 locations simultaneously | CODE-FIXED, VALIDATION PENDING | New `OverviewPanel.jsx` intentionally omits name/slug/type/family (already in SmartHeader). Shows cadence, management_mode, is_active, and readiness sections summary only. Branch: `fix/h3-ei-ux-cleanup`. |
...
| D3 | "Phases needing dates" is filtered duplicate of main phases list | CODE-FIXED, VALIDATION PENDING | Each phase row in the worklist now resolves `phase.exam_cycle_id ?? phase.cycle_id` against the `cycles` array and renders a `data-testid="worklist-cycle-{phase.id}"` label with cycle name + year. Phases with no matching cycle omit the label gracefully. Branch: `fix/h3-ei-ux-cleanup`. |
...
|---|---|---|---|---|
| I1 | ReviewQueueTable "Row id" button | `ReviewQueueTable.jsx:92` | CODE-FIXED, VALIDATION PENDING | `{r.id}` → `{humanizeToken(r.id)}` via `operatorChrome.humanizeToken`. `aria-label` also humanized. Raw UUID preserved in `data-testid` for e2e. Branch: `fix/h3-ei-ux-cleanup`. |
| I2 | SetupPanel phase error message | `SetupPanel.jsx:803` | CODE-FIXED, VALIDATION PENDING | Phase error copy now uses `safePhaseLabel(phase)` from `operatorChrome.js` (returns `"this phase"` instead of raw UUID). Branch: `fix/h3-ei-ux-cleanup`. |
