...
| 14-day shadow gate (PR-7) | INSUFFICIENT_DATA | Window never started: PR-6 PASS required but PR-6 failed at Gate 9. Fingerprint also changed between PR-6 baseline and current HEAD (PR #723 modified `shadow_analysis.py`; PR #726 modified `mastery_writer.py` + `mocks.py`). CLI exited 2 (ERROR) in agent environment. See `docs/audits/mastery-shadow-14day-gate-2026-06-20.md` and `docs/ops/pr7_shadow_gate_results.md`. |
