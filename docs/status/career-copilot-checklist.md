...
| M3 | PYQ questions: all 200 loaded simultaneously, no pagination | CODE-FIXED, VALIDATION PENDING | `limit=200` removed. `PyqPaperWorkspace` now fetches `PAGE_SIZE=50` per page with `limit`/`offset` server params. `reviewer_status` filter moved server-side; `source_kind` remains client-side (not supported by backend). Client-only sorts (`confidence_asc`, `status`) dropped — server orders by `question_number ASC`. Offset resets on filter/paper change; page clamped after mutations; questions refetched after review actions; total from server shown in list header. Pagination controls (prev/next, range label) added to left pane. Targeted tests: `PyqPaperWorkspace.pagination.test.jsx`. |
