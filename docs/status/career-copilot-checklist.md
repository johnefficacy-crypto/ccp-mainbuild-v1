...
| CL-5 one-primary-per-screen buttons | CLEANUP PENDING | Exam Registry is already reviewed: it has one primary header action, Open console; Create exam is secondary and Advanced import / repair remains tertiary. Remaining locked scopes: B3d-2 Console Work Queue action hierarchy, B3d-3 Guided Exam Wizard primary-action hierarchy, and B3d-close final audit. Rule: a screen may expose at most one screen-level primary CTA; pressed filters/selectors are not primary actions; repeated row actions are contextual; local form submission buttons are scoped to their form/card and are not automatically competing screen-level CTAs. `SetupPanel` redesign remains owned by Lane C and must not be absorbed into B3d. |
