...
| I8-C — Advanced Repair isolation | CODE-FIXED, VALIDATION PENDING — branch `feat/i8c-advanced-repair-isolation` | Overflow action in Manage Exam (`AdvancedRepairMenu`); `exam_intelligence.cms` permission gate; `AdminSafetyBanner` replaces amber banner; scoped list requests (exam_id / exam_cycle_id); `/console` redirect to `/exam-intelligence`; navContract updated; tests added. PR to be opened. |
