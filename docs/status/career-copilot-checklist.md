...
| 14-day shadow gate (PR-7) | NOT STARTED — BLOCKED ON PR-6 | PR-6 did not pass (Gate 9 hard stop: allowlist not deployed); no observation window opened; candidate fingerprint superseded since PR-6 inspection (PRs #723 and #726 modified fingerprinted files); no thresholds evaluated; new baseline and exact UTC window_start required after PR-6 PASS. See `docs/audits/mastery-shadow-14day-gate-2026-06-20.md` and `docs/ops/pr7_shadow_gate_results.md`. |
...
| CL-5 one-primary-per-screen buttons | CLEANUP PENDING | Exam Registry now has one primary header action: Open console. Create exam is secondary and Advanced import / repair remains tertiary. Other screens must be reviewed separately under B3d before CL-5 can close. |
