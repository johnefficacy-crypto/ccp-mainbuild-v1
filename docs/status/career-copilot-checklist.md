...
| I8-C — Advanced Repair isolation | IN PROGRESS — PR #759 open (draft) | Overflow action in Manage Exam (`AdvancedRepairMenu`); `exam_intelligence.cms` permission gate; `AdminSafetyBanner` replaces amber banner; scoped list requests (exam_id / exam_cycle_id); `/console` redirect to `/exam-intelligence`; navContract updated. BLOCKER 1: auth hydration fix (isAuthorized + scope in effect deps). BLOCKER 2: ExamIntelDocuments scope props (scopeExamId/scopeCycleId). Both fixed in current iteration — regression tests for hydration, scope change, and documents scope propagation being added. |
