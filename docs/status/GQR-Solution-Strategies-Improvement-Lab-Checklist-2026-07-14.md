...
Tests: `app/backend/tests/study_os/test_improvement_lab.py` (17 — scope, evidence, verified-only, not-a-library-dump, subject filter, wrong-first ranking, governance strip, empty, owner/response + strategy read-failure propagates, strongest-relevance both orders, recent-first sources, determinism, recency-bounded-window overflow both orders, endpoint wiring + owner/strategy 502), FE `StrategyFeedSection.test.jsx` (4, incl. worked_example) + `ImprovementLab.test.jsx` updated.
