...
> **Slice H — projected-PYQ practice→submit→review e2e (tests/fixtures only, CODE-LANDED, VALIDATION PENDING).** Closes the deferred half of this exit gate — the workspace seed had no projected verified-PYQ pool, so the full "complete and review a verified previous-year paper" path was never exercised end-to-end. New `app/frontend/e2e/fixtures/seedProjectedPyq.ts` seeds, on the E2E workspace exam, a **verified** `pyq_papers` row + verified `pyq_questions` (printed order via `question_number`/`display_order`) + primary verified `pyq_question_topic_tags` + `mock_question_bank` rows (`reviewer_status='verified'`, `pyq_question_id`/`pyq_paper_id` set, a launch-ready MCQ snapshot: 4 options + `correct_option_id`) + `mock_question_options` (printed `source_label`/`display_order`, option_index 1 correct) + **ACTIVE** `pyq_mock_question_projections` — exactly the guards `start_pyq_practice` and `practice_ready_counts_by_paper` enforce. Fixed UUIDs + service-role upserts (idempotent); `resetPyqPracticeAttempts` clears the learner's prior `pyq_practice_*` blueprint attempts. New `app/frontend/e2e/flows/pyq-practice-review.spec.ts` drives the learner path: Exam Intelligence detail → PYQ Explorer overview (`/pyq-summary`) → launch-accurate `pyq-paper-practice-btn` → `POST /practice/start` → shared attempt shell → answer all → submit → result → review with the projected correct answer (printed label, never a UUID). Also validates the live-schema path (migration 231 blueprint source, 183/229 projection bridge) that mock-Supabase unit tests cannot. Verified: `(cd app/frontend/e2e && npx tsc --noEmit)` → exit 0. **VALIDATION PENDING:** not yet run against a live Supabase/backend here — CI e2e run is the remaining proof. **Revision-mode entry stays deferred** (SRS/mastery due-signals, PR-8; mastery live-write gate closed), so the exit gate is not yet fully code-complete.
