...
Status: **PARTIAL — full-paper/section/topic API + core learner flow OPERATOR VALIDATED (2026-07-13, UPSC CSE); three UI-quality defects OPEN; revision mode DEFERRED (PR-8/SRS)** — slice A (render fidelity) + slice B (practice attempt assembly, migration 231) + slice C (learner launcher `PyqExplorerSection`) + slice D (learner practice UX hardening, PR #940); reuses the mock attempt shell. The source-of-record checklist tracks the combined PR-5/6 row as `IN PROGRESS` for the same reason — the revision entry mode is not yet built and the UI defects below are open, so the full learner-practice exit gate is not code-complete.

> **Live operator validation (2026-07-13) — API + CORE LEARNER FLOW VALIDATED; UI defects open.** Full record: `docs/audits/2026-07-13-upsc-cse-pyq-learner-flow-operator-validation.md`. The `exam_phases.name` vs canonical `phase_name` bug (`42703`, HTTP 200 empty payloads on `/pyq-summary` and `/pyqs`) was fixed in **PR #980** (merged to `main`) and confirmed live after redeploy: `pyq-summary` → `papers:4, questions:177, projected_practice_ready:177, error:null`; `pyqs` → `total:177, error:null`, first item carries `phase_name:"Prelims"` + subject/primary-topic metadata + 4 options + correct option id. Data state: 4 verified papers, 177 verified questions each with exactly one verified primary tag, 177 active projections (2 stale refreshed pre-validation); GS 97 + CSAT 80 = 177. Learner flow end-to-end PASS: Explorer totals, launch-accurate Practice buttons, 97-question GS paper launch, question render, answer persistence, submission (no failed-save/API error), result analytics (Overview/Topic/Time/Error), and review of all 97 questions. Disposition: `API AND CORE LEARNER FLOW VALIDATED / FULL-PAPER PRACTICE FUNCTIONAL / FOLLOW-UP UI DEFECTS OPEN`. Three frontend defects remain open (tracked in Slice I below): attempt-header timer shows `--`; navigator clips Q96–97 behind the fixed footer + no auto-scroll/active-sync under keyboard nav; review options concatenated without separation.

> **Slice I — post-validation learner-UX follow-ups (OPEN, frontend + 1 e2e; from 2026-07-13 operator validation).** Focused frontend PR to close the three UI-quality defects the full-paper attempt/review surfaced (data pipeline, API correction and main workflow are already validated — these are usability, not data-integrity, blockers). (1) **Attempt timer `--`** [High] — the header shows `--` during a valid timed attempt though per-question dwell tracking works (Time result tab correctly reported total 18m 59s / avg 12s). Bind the timer to the launch-payload duration; initialize only after attempt data loads; render an explicit `Untimed` fallback when no duration exists; never show `--` on a valid timed attempt; add a regression asserting a valid time value after load. (2) **Navigator clips Q96–97** [High] — the right-side palette visually ends at Q95 because its lower rows sit behind the fixed action footer; Q96/97 are keyboard-reachable but not visible, the palette does not auto-scroll to the active question, and the active highlight does not reliably follow arrow-key nav. Give the navigator its own vertical scroll container reserving footer height; call `scrollIntoView({ block: "nearest" })` for the active palette item on index change; derive active state from the canonical current question id/index; test keyboard, mouse and direct-palette nav on a >95-question paper. (3) **Review options concatenated** [Medium–High] — review renders `A. …B. …C. …D. …` with no separation between label and text. Render each option as a separate block/semantic list item with label–text spacing, reusing the shared option-label formatter already used in the attempt shell; verify multiline wrapping; add a review-render test asserting each option renders in its own element. (4) **97-question e2e regression** — extend `app/frontend/e2e/flows/pyq-practice-review.spec.ts` with a ≥97-question paper asserting a valid timer / explicit untimed state, keyboard nav to Q97 with the palette item visible + active, all 97 review navigator items present, and options rendered as separate readable elements — guarding against sticky-header/footer/navigator viewport clipping.
