...
**Delivered follow-up (2026-07-10) — Mission Control EWP task shaping (MERGED #941, LIVE VALIDATED).** Live validation of the planner-launch path (Follow-up B in the checklist) surfaced a Study Home CTA bug: Mission Control only forwarded `launch_type` into `today_tasks` when `writing_practice/launch.compute_action` returned an action, and `compute_action` required a non-null `launch_entity_id`. A planner-shaped writing task has no pre-existing session (`launch_entity_id` is null), so the task lost its `launch_type` and Study Home rendered the generic "Start → /app/study/plan" CTA instead of `LaunchWritingPracticeButton`. Fix: `compute_action` resolves for any `english_writing_session` launch, returning `action_label` from `launch_context.exercise_type` and `action_url=None` when no session exists (the click creates the session via `POST /api/study/tasks/{id}/launch-writing`); `_load_today_tasks` then preserves `launch_type` on planner-shaped tasks. Scope: `launch.py`, `mission_control.py`, `test_writing_launch.py`, `test_mission_control.py`, `StudyHome.writingLaunch.test.jsx`. No DB/migration/RLS change; the frontend contract is unchanged (button still launches through the server-owned endpoint). **Merged in #941 and now live-validated end-to-end:** an operator-created planner-shaped task (`launch_entity_id=NULL`) renders the `Start sentence practice` CTA on Study Home and clicking it launches, creates `session_id=3c90846c-20b6-43d4-8f97-bdf035f1f948`, and completes evaluation (see checklist Follow-up B). The launch/runtime plumbing is complete; the remaining EWP work is planner **generation** of `english_writing_session` tasks for SSC/UPSC/CSAT English (a separate feature, not launch wiring).
