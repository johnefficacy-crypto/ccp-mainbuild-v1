...
| **OD-5** | Backfill posture for existing manual `locked` coverage rows: leave untouched (PD-4) vs. produce shadow `draft` derived rows alongside for operator comparison. | Grandfathering vs. re-review sweep is the same class of decision as the prerequisite gate PD-D; operator must choose. |
