...
| PD-4a | **Row-ownership / conflict model (LOCKED — added per checkpost; corrects the PD-4 gap that only protected reviewed/locked).** The `exam_topic_coverage` unique index allows exactly ONE row per canonical scope/topic, so a derived row cannot coexist beside an existing row for the same scope without a schema change. Therefore: (1) the derivation may write/update a row ONLY when that row is **derivation-owned** — identified by `source_basis ∈ {derived provenance set}` AND `model_version` set by the derivation (i.e. a row it previously created); (2) a row in ANY state (incl. `draft`) that is human/manual-authored (`source_basis ∈ {manual, admin_review}`) is NEVER overwritten — a manual `draft` is protected exactly like a locked row; (3) if the canonical scope/topic already has a manual or reviewed/locked row, the derivation does NOT create a second row (uniqueness forbids it) — it records a **delta** (see PD-4b) instead of upserting. |
