...
## Section F — OPERATOR DECISION REQUIRED

| ID | Decision needed | Why it cannot be guessed |
|---|---|---|
| **OD-1** | Final canonical category enum (does it include `pwd`, `ex_servicemen`, PwBD sub-types, state-domicile categories?) and must it be byte-identical to the eligibility engine's category vocabulary. | Determinism requires ONE category taxonomy; picking the wrong set silently mis-buckets cutoffs. Cross-domain — needs product/eligibility owner. |
| **OD-2** | Enforcement point + rule for barring `source_basis='model_generated'` rows from reaching `reviewed`/`locked` without human evidence. | Touches the "no new AI writes / verified-only" invariant; the exact gate (submit vs. promote) is a governance call. |
| **OD-3** | Is a qualitative cutoff/difficulty direction (`rising`/`flat`/`falling`) a product requirement? If yes, confirm it is DERIVED at read time, never stored. | The current UI stores it as truth; retiring it changes operator workflow. |
| **OD-4** | On `sum(vacancy_by_category) != vacancy_total`: warn-only, or hard-reject at write/promote? | Data-quality strictness is a policy trade-off (real notifications sometimes omit categories). |
| **OD-5** | Legacy-row handling: normalize existing `cutoff_trend` string/number/list rows to the B.1 object shape (and how), or grandfather them exempt from the new constraint? | Mirrors J2-A′ PD-D; must record pre/post counts. Wrong choice either breaks existing rows or leaves malformed data readable. |
| **OD-6** | Evidence model: `metadata.evidence` JSONB array vs. dedicated `exam_competition_metrics_evidence` child table; keep, derive, or drop `evidence_count`. | Audit-strength vs. cost trade-off; a child table adds RLS + migration surface. |
