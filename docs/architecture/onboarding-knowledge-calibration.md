...
- **Resolved once, no gate→planner TOCTOU.** `_calibration_gate_response` resolves the target ONCE and returns the `exam_id`; the route passes it to the planner entrypoints as `expected_exam_id`, and `_compute_plan` aborts with `{generated:false, reason:"target_changed"}` if its own resolution differs. So a target switched concurrently (exam A checked, exam B resolved by the planner) can never generate/persist a plan for B whose gate was never checked. Internal/regen callers omit `expected_exam_id` (unchanged).
