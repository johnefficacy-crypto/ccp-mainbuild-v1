...
- **Fail closed on read failure.** Calibration reads are health-checked (`resolve_required_subjects` / `read_gate` / `has_existing_plan` each return an explicit `ok` flag; `evaluate_calibration` returns `check_failed`; target resolution uses `resolve_target_exam_checked`, which fails closed rather than mistaking a transient `profiles`/`preferences` read for "no target exam"). If a read needed to make the decision fails, the state is **unknown** and the server does NOT collapse that to "nothing to calibrate" (which would silently unlock). `calibration_required` raises `CalibrationUnavailable`; the plan endpoints return **HTTP 503 `calibration_check_failed`**, `GET` returns `calibration_check_failed:true` (never a bogus `calibrated`), `PUT`/`skip` return 503, and `regen` skips. The frontend shows a retry, not the interstitial — including on a generic GET/network failure (the hook's catch path sets `checkFailed`). The target exam itself is resolved with a **strict, non-swallowing `exams` read** (`resolve_target_exam_checked`), not the shared `resolve_exam_by_*` helpers — those `_safe`-swallow and negative-cache a miss, which would fail open for the cache TTL.
- **Resolved once, no gate→planner TOCTOU.** `_calibration_gate_response` resolves the target ONCE and returns the `exam_id`; the route passes it to the planner entrypoints as `expected_exam_id`, and `_compute_plan` aborts with `{generated:false, reason:"target_changed"}` (mapped to HTTP 422) if its own resolution differs. So a target switched concurrently (exam A checked, exam B resolved by the planner) can never generate/persist a plan for B whose gate was never checked. The **no-target → exam B** race is covered too: when the gate finds no target it passes `NO_TARGET_SENTINEL` (not `None`) as `expected_exam_id`, so any exam that appears before the planner resolves is rejected as `target_changed`; a genuinely still-no-target user gets `no_target_exam`. Internal/regen callers omit `expected_exam_id` (unchanged).
- **Positive evidence wins — no false re-block.** `evaluate_calibration` checks POSITIVE unlock evidence FIRST (completed/skipped gate, then existing plan) before resolving the required set, so an unrelated coverage / topics / mastery / subjects outage can never re-block an already-unlocked user (the OR contract). `has_existing_plan` matches the canonical `exam_id` BEFORE the `exams` slug lookup (a transient `exams` read can't block a UUID-matching plan). For an already-unlocked user, `needs_update` and the editable `required_subjects` are computed best-effort — failure to recompute them never revokes access. GET prefill (evidence rows, subject names) is likewise best-effort and never changes `calibrated`.
