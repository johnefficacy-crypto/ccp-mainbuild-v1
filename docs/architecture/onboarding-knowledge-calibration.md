...
- **Rollout / grandfather.** `calibration_required` is False when the user already has a plan for the exam, so users who generated a first plan before this feature shipped (no gate row) are **not** retroactively blocked from regenerating. The gate applies only to a genuine first plan, and the same rule backs `GET`'s `calibrated`, so the interstitial never appears for an existing-plan user.
