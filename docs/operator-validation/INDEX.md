...
| **UPSC Mains coverage consolidated onto the plannable phase**<br>`cov-phase-01-mains-coverage-consolidation` | **OPERATOR PENDING** | 2026-09-14 | 2026-09-30T18:00:00Z | `cov-phase-01-duplicate-coverage-rows-in-learner-reads` Every exam-wide learner read offered each Mains topic twice, once per phase — the palette, the calibration subject set and the Subject Hub. Reported live as 'Balance of power: methods and contemporary relevance', 9.20 twice.<br>`cov-phase-01-180-topics-only-on-the-template-phase` 180 topics carry locked coverage only on the cycle-less template phase, which exam_target_window can never target, so the planner cannot schedule them. | `cov-phase-01-duplicate-coverage-rows-in-learner-reads` load_scoped_coverage[_checked] resolves to one row per topic, preferring the cycle-attached phase, then exam-wide, then template, with coverage_id breaking ties. A failed exam_phases read leaves the duplicates and reports the read unhealthy rather than keeping the wrong row. | Run workbench/sql/COV-PHASE-01_orphans.sql §1-§4 against the live database, then execute docs/runbooks/cov-phase-01-mains-coverage-consolidation.md against UPSC CSE: compute and lock cycle-phase score snapshots where §3 reports them missing, derive coverage scoped to the cycle phase, review and lock, and attach one evidence record with §2 and §6 re-run. | [cov-phase-01-mains-coverage-consolidation.md](../../docs/runbooks/cov-phase-01-mains-coverage-consolidation.md) | — |
