...
| P6 | Scheduler verification (jobs/manual-run/drain) | OPS | ✅ OPERATOR PASS | Candidate SHA `b9bd9d7b`: startup + sweeper registration + repeat sweeps ✅; `/api/admin/jobs` response shape confirmed (no `enabled` field); controlled `analytics_retry` drain proof complete (job `1afa0c0a`, status=done, attempts=1, last_error=null). | `audits/2026-07-01-scheduler-drain-validation.md` |
...
3. **OPS:** ~~P6 scheduler drain~~ **DONE** (candidate SHA `b9bd9d7b`; see `audits/2026-07-01-scheduler-drain-validation.md`) → deploy current `main` to staging (record candidate SHA A; require Render deployed SHA B == A) → **P5** PR #800 validation + boundary approval → **P7** PR-6 rerun on the pinned deployed SHA.
