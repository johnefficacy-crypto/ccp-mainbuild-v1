...
- **EWP sentence-practice slice:** `sentence_construction` live runtime **VALIDATED end-to-end** (Content Studio seed → verify → activate → session → submit → evaluate → `completed`), and the planner-launch **backend/runtime path is now live-validated** (`POST /api/study/tasks/{id}/launch-writing` → server-side prompt resolution → session creation → practice load → submit → evaluation → completion; `study_task_id=4fa0a619…`, `session_id=3c22ef9e…`). Migration 238 (`completed_at` rollup) confirmed live; `submitted_at` remains an open data-quality gap. **Remaining code gap for a UI-driven Study Home click-through:** the Mission Control EWP task-shaping fix (`compute_action`/`_load_today_tasks` preserve `launch_type` on planner-shaped tasks whose `launch_entity_id` is null) — `CODE-FIXED, VALIDATION PENDING`, unit-tested, no migration. After it merges the Study Home CTA can be validated from the UI without a direct console POST. UPSC/CSAT lacks the button by design until the planner auto-creates `english_writing_session` tasks for those exams. Source-of-record: `docs/status/career-copilot-checklist.md` § EWP `sentence_construction` live runtime validation.
