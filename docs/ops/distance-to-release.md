...
- **EWP sentence-practice slice — LIVE VALIDATED end-to-end (launch/runtime plumbing complete):** `sentence_construction` live runtime validated (Content Studio seed → verify → activate → session → submit → evaluate → `completed`); the planner-launch backend/runtime path validated (`POST /api/study/tasks/{id}/launch-writing` → server-side prompt resolution → session creation → practice load → submit → evaluation → completion); and — after the Mission Control task-shaping fix landed (**#941**, merged) — the **UI-driven Study Home click-through is now live-validated**: an operator-created planner-shaped task (`launch_type='english_writing_session'`, `launch_entity_id=NULL`) renders the `Start sentence practice` CTA, and clicking it launches through the server-owned endpoint, creates the session, opens the practice route, and completes evaluation (`study_task_id=05988024…`, `session_id=3c90846c…`, `completed_at=2026-07-10T15:07:27Z`). Migration 238 (`completed_at` rollup) confirmed live; `submitted_at` remains a separate open data-quality gap. **Remaining product gap (not launch plumbing):** planner **generation** does not yet auto-create `english_writing_session` tasks for SSC/UPSC/CSAT English — that is the next EWP feature, distinct from the now-validated launch/runtime path. Source-of-record: `docs/status/career-copilot-checklist.md` § EWP `sentence_construction` live runtime validation.
