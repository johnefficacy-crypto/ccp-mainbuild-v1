...
CREATE TABLE public.organizations (
  ...
  verified_by uuid,
  calendar_status text CHECK (calendar_status = ANY (ARRAY['published'::text, 'tentative'::text, 'partial'::text, 'needs_review'::text])),
  metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
  short_name text,
...
CREATE TABLE public.scrape_runs (
  ...
  discovery_items_found integer NOT NULL DEFAULT 0,
  discovery_items_new integer NOT NULL DEFAULT 0,
  discovery_items_lifecycle integer NOT NULL DEFAULT 0,
...
CREATE TABLE public.scrape_queue (
  ...
  extraction_status text NOT NULL DEFAULT 'unverified'::text,
...
CREATE TABLE public.courses (
  ...
  delivery_model text NOT NULL DEFAULT 'platform_course'::text CHECK (delivery_model = ANY (ARRAY['affiliate_external'::text, 'platform_course'::text, 'platform_download'::text, 'platform_test'::text, 'platform_bundle'::text])),
  ...
  CONSTRAINT courses_affiliate_partner_fkey FOREIGN KEY (affiliate_partner_id) REFERENCES public.affiliate_partners(id)
);
...
CREATE TABLE public.mock_tests (
  ...
  source_type text NOT NULL DEFAULT 'manual_log'::text CHECK (source_type = ANY (ARRAY['manual_log'::text, 'platform_attempt'::text, 'imported_result'::text])),
  trust_level text NOT NULL DEFAULT 'self_reported'::text CHECK (trust_level = ANY (ARRAY['self_reported'::text, 'platform_verified'::text, 'admin_verified'::text])),
  ...
  CONSTRAINT mock_tests_mock_attempt_id_fkey FOREIGN KEY (mock_attempt_id) REFERENCES public.mock_attempts(id)
);
...
CREATE TABLE public.mentor_bookings (
  ...
  agenda text,
  status text NOT NULL DEFAULT 'requested'::text CHECK (status = ANY (ARRAY['requested'::text, 'pending_payment'::text, 'awaiting_mentor'::text, 'confirmed'::text, 'completed'::text, 'cancelled'::text, 'no_show'::text, 'refunded'::text])),
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  mentor_slug text,
  ...
  notes text,
  payment_id text,
  payment_status text NOT NULL DEFAULT 'unpaid'::text CHECK (payment_status = ANY (ARRAY['unpaid'::text, 'authorized'::text, 'captured'::text, 'failed'::text, 'refunded'::text])),
  metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
...
CREATE TABLE public.recruitment_events (
  ...
  event_hash text DEFAULT encode(sha256((((((COALESCE((source_id)::text, ''::text) || '|'::text) || COALESCE(event_type, ''::text)) || '|'::text) || COALESCE((payload ->> 'discovered_url'::text), ''::text)))::bytea), 'hex'::text),
