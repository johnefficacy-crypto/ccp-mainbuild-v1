...
**Mode:** read-only auditor. This is a separate audit document created instead of directly updating the existing `operator_entity_coverage.md`. The graph was used only for orientation after reading `graphify-out/GRAPH_REPORT.md`; every matrix cell below is based on live source files.
