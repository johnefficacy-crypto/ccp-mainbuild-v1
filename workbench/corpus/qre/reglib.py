...
class Batch:
    ...
    def add(self, micro, level, stem, correct, wrongs, steps, formula, trap,
            kind="numerical", group=None, verify_fact=False, ref=None, tier=None):
        qid = f"{self.prefix}-{len(self.Q)+1:03d}"
        assert tier in ("foundation", "officer"), f"{qid}: tier must be foundation|officer"
        ...
        self.Q.append({
            ...
            "rubric_level": level, "difficulty": LEVEL_DIFF[level], "exam_tier": tier,
    ...
    def write(self):
        ...
        for q in self.Q:
            flag = (" ⚠" if q["verify_fact"] else "") + f" · {q.get('exam_tier')}"
